...
public class SimulatorServiceImpl implements SimulatorService {
    ...
    @NotNull
    @Override
    public Pair<Thread, Future<List<ReturnDTO>>> simulate(@NotNull JsonNode body) {
        CompletableFuture<List<ReturnDTO>> future = new CompletableFuture<>();
        Thread thread = new Thread(() -> {
            ...
            for (final JsonNode line : body.get("canvas").get("lines")) {
                ...
                channelNameEndPointMap.put("c" + idCounter, new LineCoords(
            ...
            List<ReturnDTO> returnStates = states.stream()
                .map(ReturnDTO::new)
            ...
            future.complete(returnStates);
        });
        thread.start();
        return new Pair<>(thread, future);
    }
