...
function formatVolume(volume: number): string {
    ...
    return volume.toFixed(volume.toString().length - 2);
}
...
jQuery((): void => {
    $('#newDropletModal').on('shown.bs.modal', (): void => {
    ...
    $('#newDropletModal').on('submit', (event: SubmitEvent): void => {
        ...
        let newDroplet = new Droplet(nextId++, newDropletName.val().toString(), Number(newDropletVolume.val()));
